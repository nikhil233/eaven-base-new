...
func ChannelRoutes(router *mux.Router) {
	...
	// Team routes
	protectedRouter.HandleFunc("/create", channelService.CreateChannel).Methods(http.MethodPost, http.MethodOptions)
	// protectedRouter.HandleFunc("/all", channelService.GetUserTeams).Methods(http.MethodGet)
	protectedRouter.HandleFunc("/get/{id}", channelService.GetChannel).Methods(http.MethodGet, http.MethodOptions)
	...
	protectedRouter.HandleFunc("/{channel_id}/join", channelService.SubscribeChannel).Methods(http.MethodPost, http.MethodOptions)
	protectedRouter.HandleFunc("/message", messageService.SendMessage).Methods(http.MethodPost, http.MethodOptions)
}
